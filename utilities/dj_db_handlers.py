...
def addExpenses(expensedata, year="2024",mytable=mytable):
    ...
    insertString = "INSERT INTO {0} (date, charge_name, amount, tag, notes) VALUES ({1})" .format(mytable, expensedata)
...
def queryByMonth(month,year="2024", mydb=mydb, mytable=mytable):
    ...
    monthQuery = "SELECT date, charge_name, amount, tag, notes FROM {0} WHERE date LIKE '{1}%'".format(mytable,month)
    try:
...
def queryByYearlyTable(mytable=mytable,year="2024", mydb=mydb):
    ...
    recordquery = "SELECT date, charge_name, amount, tag, notes FROM {0}".format(mytable)
    # print(recordquery)
    try:
