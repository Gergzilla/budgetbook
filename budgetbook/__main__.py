...
def main():
    ...
    ttk.Button(mainframe, text="View Expense Summary", command="").grid(
        column=8, row=2, sticky=(E, S)
    )
    ttk.Button(mainframe, text="View Budget Report", command="").grid(
        column=8, row=3, sticky=(E, S)
    )
    ttk.Button(mainframe, text="Print Expenses", command=printBoxContents).grid(
        column=8, row=4, sticky=(E, S)
    )
    ttk.Button(mainframe, text="Import Expenses", command=importFile).grid(
        column=7, row=5, sticky=(E, S)
    )
    ttk.Button(mainframe, text="Save Expenses", command=saveExpenses).grid(
        column=8, row=5, sticky=(E, S)
    )
    ttk.Button(mainframe, text="Quit", command=leave).grid(
    ...
    ttk.Button(mainframe, text="delete duplicates", command=deleteDupes).grid(
